...
from processor.enhanced_processor import enhanced_do_train, enhanced_do_inference
...
if __name__ == '__main__':
    ...
    if getattr(args, 'use_enhanced_rde', False):
        enhanced_do_train(start_epoch, args, model, train_loader, evaluator, optimizer, scheduler, checkpointer)
    else:
    ...
    for i in range(len(asss)):
        if os.path.exists(op.join(args.output_dir, asss[i])):
            ...
            model = model.cuda()
            # Choose inference function based on model type
            if getattr(args, 'use_enhanced_rde', False):
                enhanced_do_inference(model, test_img_loader, test_txt_loader)
            else:
