...
def build_CLIP_from_openai_pretrained(name: str, image_size: Union[int, Tuple[int, int]], stride_size: int, jit: bool = False, download_root: str = None):
    ...
    # Ensure image_resolution is always a tuple (height, width)
    if isinstance(image_size, int):
        # For Re-ID task, commonly use (384, 128) format (height, width)
        if image_size == 384:
            model_cfg['image_resolution'] = (384, 128)
        else:
            # For other sizes, assume square images
            model_cfg['image_resolution'] = (image_size, image_size)
    else:
